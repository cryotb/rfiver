...
#define TBX_XSTR_SEED (3600ull)
  ...
  // @return a pseudo random number clamped at 0xFFFFFFFF
    ...
    {
        return 1013904223ull + (1664525ull * ((rounds > 0) ? linear_congruent_generator(rounds - 1) : (TBX_XSTR_SEED)))
            % 0xFFFFFFFF;
    }
...
#define XSTR_RANDOM_NUMBER(Min, Max) (Min + (Random() % (Max - Min + 1)))
  ...
  // -----------------------------------------------------------------------------
    ...
    template <unsigned size, typename uint64_t>
    class Xor_Addr
    {
    public:
        const unsigned _nb_chars = (size - 1);
        ...
        // if every goes alright this constructor should be executed at compile time
        constexpr Xor_Addr(const uint64_t addr)
            : _addr{}
        {
            _addr = (addr ^ XORKEY);
        }
...
#define XOR_S(name, my_string)    constexpr crypt::Xor_string<(sizeof(my_string)/sizeof(char)), char> name(my_string)
...
#define _XS(my_string) []()[[]]{ constexpr crypt::Xor_string<(sizeof(my_string)/sizeof(char)), char> expr(my_string); return expr; }().decrypt()
#define _XA(my_addr)[]{ constexpr crypt::Xor_Addr<(sizeof(my_addr)/sizeof(uint64_t)), uint64_t> expr(my_addr); return expr; }().decrypt()

// Crypt normal string char*
#define XOR_C( string ) XorString( string )

#define XOR_WS(name, my_string)       constexpr crypt::Xor_string<(sizeof(my_string)/sizeof(wchar_t)), wchar_t> name(my_string)
#define _XSW(my_string) []()[[]]{ constexpr crypt::Xor_string<(sizeof(my_string)/sizeof(wchar_t)), wchar_t> expr(my_string); return expr; }().decrypt()

// crypt  wide characters
#define XOR_W( string ) XorWideString( string )
#define XOR_STD_STRING(X) XOR_STRING(X)

constexpr uint64_t __xor_constant_key = 0xF2E1062D448B;
#define _XC(X) (X ^ __xor_constant_key)
